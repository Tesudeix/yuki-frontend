...
type User = { _id?: string; id?: string; name?: string; username?: string; phone?: string; avatarUrl?: string };
...
export default function FeedPostCard({ post, onDelete, onShareAdd }: Props) {
  ...
  const isSuperAdmin = (digits(user?.phone) && digits(user?.phone) === digits(ADMIN_PHONE)) || (digits(user?.username) && digits(user?.username) === digits(ADMIN_PHONE));
  ...
  const letter = (state.user?.name || state.user?.username || state.user?.phone || "U").slice(0, 1).toUpperCase();

  // removed unused UTC formatter

  const username = (state.user?.name || state.user?.username || state.user?.phone || "user").toString().slice(-6);
      ...
      <header className="flex items-start gap-3">
        ...
        <div className="flex-1">
          ...
          <div className="flex items-center justify-between">
            <div>
              <p className="font-semibold text-white leading-none">{state.user?.name || state.user?.username || state.user?.phone || "User"}</p>
        ...
        <section className="grid gap-2 pt-2">
          {(state.comments || []).map((c) => (
            <div key={c._id} className="text-sm">
              <div className="font-semibold">{c.user?.name || c.user?.username || c.user?.phone || "User"}</div>
              <div className="text-[13px]">{c.content}</div>
              {(c.replies || []).map((r) => (
                <div key={r._id} className="ml-4 text-xs mt-1">
                  <div className="font-semibold">{r.user?.name || r.user?.username || r.user?.phone || "User"}</div>
                  <div>{r.content}</div>
                </div>
              ))}
